  ...
  >
    ...
    <el-card class="box-card">
      ...
      <div v-loading="loading" class="photo-list">
        <el-checkbox-group v-for="item in photoList" :key="item.id" v-model="selectedIds">
    ...
    >
      <el-form label-width="80px">
        <el-form-item label="目标相册">
          <el-select
            v-model="moveDialog.targetAlbumId"
            placeholder="请选择目标相册"
            style="width: 100%"
          >
            <el-option
...
const rules = reactive<FormRules>({
  ...
  recordTime: [{ required: false, message: '请选择记录的日期', trigger: 'blur' }],
...
const submitForm = async () => {
  ...
  const valid = await photoFormRef.value.validate().catch(() => false)
  if (!valid) return
