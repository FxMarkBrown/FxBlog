...
const handleBatchDelete = async () => {
  ...
  try {
    await ElMessageBox.confirm(`是否确认删除 ${selectedIds.value.length} 个说说?`, '警告', {
...
const handleDelete = async (row: any) => {
  try {
    await ElMessageBox.confirm(`是否确认删除 ${row.content} 这个说说?`, '警告', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    })
...
const submitForm = async () => {
  ...
  const valid = await momentFormRef.value.validate().catch(() => false)
  if (!valid) return
