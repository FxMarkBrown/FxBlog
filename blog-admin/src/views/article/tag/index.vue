    ...
    <el-card class="box-card">
      <template #header>
        <div class="card-header">
          <ButtonGroup>
            <el-button v-permission="['sys:tag:add']" type="primary" icon="Plus" @click="handleAdd"
              >新增</el-button
            >
            <el-button
    ...
    >
      ...
      >
        ...
        <el-form-item label="排序" prop="sort">
          <el-input-number v-model="tagForm.sort" :min="1" placeholder="请输入排序" clearable />
        </el-form-item>
      </el-form>
...
const handleBatchDelete = async () => {
  ...
  try {
    await ElMessageBox.confirm(`是否确认删除 ${selectedIds.value.length} 个标签?`, '警告', {
...
const handleDelete = async (row: any) => {
  try {
    await ElMessageBox.confirm(`是否确认删除 ${row.name} 这个标签?`, '警告', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    })
