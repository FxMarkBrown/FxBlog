<template>
  ...
  import {deleteMessageApi, getMessageListApi} from '@/api/message/message'
  ...
  const handleBatchDelete = async () => {
    ...
    try {
      await ElMessageBox.confirm(`是否确认删除 ${selectedIds.value.length} 条留言?`, '警告', {
  ...
  const handleDelete = async (row: any) => {
    try {
      await ElMessageBox.confirm(`是否确认删除 ${row.nickname} 的留言?`, '警告', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      })
