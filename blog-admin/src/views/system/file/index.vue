    ...
    <el-card class="box-card">
        ...
        <div class="card-header file-toolbar">
          <ButtonGroup class="file-toolbar-group">
            ...
            <el-input
              ...
              clearable
            />
            <el-upload :show-file-list="false" :http-request="handleManualUpload" :multiple="true">
      ...
      <el-table v-loading="loading" :data="fileList" style="width: 100%">
          ...
          <template #default="scope">
            <div class="editable-cell editable-cell--path">
              <div class="file-path-cell">
                {{ scope.row.basePath }}{{ scope.row.path }}{{ scope.row.filename }}
              </div>
              <el-button type="primary" link icon="Edit" @click="handleOpenRename(scope.row)" />
            </div>
          </template>
        ...
        <el-table-column label="存储平台" align="center" prop="platform" width="110">
          <template #default="scope">
            <span v-for="item in ossOptions" :key="item.value">
              <el-tag v-if="scope.row.platform === item.value" :type="item.style">
                {{ item.label }}
              </el-tag>
            </span>
          </template>
        ...
        <el-table-column label="操作" align="center" width="116" fixed="right">
          <template #default="scope">
            <div class="file-actions">
              <el-button type="success" link icon="DocumentCopy" @click="handleCopyUrl(scope.row)"
                >复制路径</el-button
              >
              <el-button
    ...
    <el-drawer v-model="drawerVisible" title="云存储配置" direction="rtl" size="40%">
      ...
      >
        ...
        <el-form-item label="域名" prop="domain">
          <el-input
            v-model="ossConfigForm.domain"
            placeholder="本地推荐填 /static/，云存储可填完整域名并以 / 结尾"
          />
        </el-form-item>
        ...
        <div v-if="ossConfigForm.platform === 'local'">
          ...
          >
            <el-input
              ...
              placeholder="例如 /static/**，要与上面的域名路径一致"
            />
          </el-form-item>
    ...
    <el-dialog v-model="renameDialogVisible" title="修改文件信息" width="520px">
      ...
      <template #footer>
        <div class="dialog-footer">
          ...
          <el-button type="primary" :loading="renameLoading" @click="handleSubmitRename"
            >保 存</el-button
          >
        </div>
      </template>
