...
const getList = async () => {
  ...
  try {
    ...
    menuList.value = []
    menuOptions.value = [{ id: 0, title: '顶级菜单', children: [] }]
  } finally {
    loading.value = false
  }
}
...
const legacyElementIconPrefix = `${['el', 'icon'].join('-')}-`

// 处理图标名称转换
const formatIconName = (icon: string) => {
  if (!icon) return ''
  if (icon.startsWith(legacyElementIconPrefix)) {
    return icon.slice(legacyElementIconPrefix.length)
  }
  // 确保首字母大写
