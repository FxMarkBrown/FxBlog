...
public class SensitiveUtil {
    ...
    @PostConstruct
    public void init() {
        InputStream is = Thread.currentThread().getContextClassLoader().getResourceAsStream(SENSITIVE_WORD);
        if (is == null) {
            is = SensitiveUtil.class.getClassLoader().getResourceAsStream(SENSITIVE_WORD);
        }
        if (is == null) {
            logger.warn("加载敏感词文件失败: 未找到 {}", SENSITIVE_WORD);
            return;
        }
        try (
