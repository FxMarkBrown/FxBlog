...
const heroBadge = computed(() => (isArticleMode.value ? '文章对话' : '全局对话'))
const heroTitle = computed(() => (isArticleMode.value ? '这篇文章的 AI 阅读助手' : '站内 AI 对话中心'))
const chatTitle = computed(() => (isArticleMode.value ? '文章上下文会话' : '全局知识会话'))
...
async function ensureConversation() {
  ...
  if (existingConversation) {
    await router.replace({ path: '/ai', query: buildConversationQuery(existingConversation) })
    return
  }
  ...
  if (conversation) {
    await router.replace({ path: '/ai', query: buildConversationQuery(conversation) })
  }
}
...
/**
 * 对模型输出做 Markdown 归一化，并在需要时修复流式拼接造成的结构塌陷。
 */
function normalizeMarkdownContent(content: string, aggressive = false) {
  if (!content) {
    return ''
  }
