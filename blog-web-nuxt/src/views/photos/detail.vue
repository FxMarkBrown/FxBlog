...
type AlbumPhotoItem = AlbumPhoto & {
  url: string
  description: string
  recordTime: string
  location: string
...
const passwordDialog = ref<InstanceType<typeof AlbumPasswordDialog> | null>(null)
const shouldPromptPassword = ref(false)

const album = reactive<AlbumDetailState>({
  id: albumId.value,
  name: '',
  description: '',
  createTime: '',
...
const photos = ref<AlbumPhotoItem[]>([])
...
function normalizePhotos(records: AlbumPhoto[]) {
  return records.map((photo) => ({
    ...photo,
    url: String(photo.url || IMAGE_ERROR_PLACEHOLDER),
    description: String(photo.description || ''),
    recordTime: String(photo.recordTime || ''),
...
function previewImage(index: number) {
  imagePreview.value?.show(images.value, index)
}
...
async function getAlbumPhotos() {
  try {
    const response = await getAlbumPhotosApi(albumId.value)
    const list = normalizePhotos(unwrapResponseData<AlbumPhoto[] | null>(response) || [])
    photos.value = list
    images.value = list.map((item) => item.url)
    isAuthenticated.value = true
    shouldPromptPassword.value = false
  } catch {
    photos.value = []
...
async function handlePasswordSubmit(password: string, done: () => void) {
  try {
    await verifyAlbumPasswordApi(albumId.value, password)
    await getAlbumPhotos()
    done()
  } catch (error) {
    showError((error as Error)?.message || '密码错误')
    passwordDialog.value?.setLoading(false)
  }
}
