...
export function getSystemNoticeApi() {
  return useApiClient()<ApiResponse<boolean>>('/notifications/is-unread')
}

// 分页获取消息通知。
export function getNotificationsApi(query: Record<string, unknown>) {
  return useApiClient()<ApiResponse<PageResult<NotificationItem>>>('/notifications/page', {
    query
  })
}
...
export function markNotificationAsReadApi(id: number | string) {
  return useApiClient()<ApiResponse<unknown>>(`/notifications/read/${id}`)
}

// 标记全部消息通知为已读。
export function markAllNotificationsAsReadApi() {
  return useApiClient()<ApiResponse<unknown>>('/notifications/read/all')
}

// 删除指定消息通知。
export function deleteNotificationApi(id: number | string) {
  return useApiClient()<ApiResponse<unknown>>(`/notifications/delete/${id}`, {
    method: 'DELETE'
  })
}
...
export function getUnreadNotificationsCountApi() {
  return useApiClient()<ApiResponse<Record<string, { num?: number }>>>('/notifications/unReadNum')
}
