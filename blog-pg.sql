...
DROP TABLE IF EXISTS sys_ai_document_node_message CASCADE;
DROP TABLE IF EXISTS sys_ai_document_node_thread CASCADE;
DROP TABLE IF EXISTS sys_ai_document_result CASCADE;
DROP TABLE IF EXISTS sys_ai_document_task CASCADE;
...
CREATE INDEX idx_sys_article_series_status_time ON sys_article (series_id, status, create_time DESC);
...
-- AI 文档任务表
CREATE TABLE sys_ai_document_task (
    id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    user_id bigint NOT NULL,
...
CREATE INDEX idx_ai_document_task_user_time
    ON sys_ai_document_task (user_id, update_time DESC);

CREATE INDEX idx_ai_document_task_remote
    ON sys_ai_document_task (remote_task_id);

CREATE INDEX idx_ai_document_task_expire
    ON sys_ai_document_task (expire_at);

-- AI 文档任务结果表
CREATE TABLE sys_ai_document_result (
...
-- AI 文档节点线程表
CREATE TABLE sys_ai_document_node_thread (
    id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    ...
    create_time timestamp DEFAULT CURRENT_TIMESTAMP,
    update_time timestamp DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT uk_ai_document_node_thread UNIQUE (task_id, user_id, node_id)
);

CREATE INDEX idx_ai_document_node_thread_user_time
    ON sys_ai_document_node_thread (user_id, last_message_at DESC);

CREATE INDEX idx_ai_document_node_thread_task_node
    ON sys_ai_document_node_thread (task_id, node_id);

-- AI 文档节点线程消息表
CREATE TABLE sys_ai_document_node_message (
    id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE INDEX idx_ai_document_node_message_thread_time
    ON sys_ai_document_node_message (thread_id, create_time);

ALTER TABLE sys_ai_document_task
    ADD CONSTRAINT fk_sys_ai_document_task_user
    FOREIGN KEY (user_id) REFERENCES sys_user(id)
    ON DELETE CASCADE
    ON UPDATE RESTRICT;

ALTER TABLE sys_ai_document_task
    ADD CONSTRAINT fk_sys_ai_document_task_source_file
    FOREIGN KEY (source_file_id) REFERENCES file_detail(id)
    ON DELETE SET NULL
    ON UPDATE RESTRICT;

ALTER TABLE sys_ai_document_result
    ADD CONSTRAINT fk_sys_ai_document_result_task
    FOREIGN KEY (task_id) REFERENCES sys_ai_document_task(id)
    ON DELETE CASCADE
    ON UPDATE RESTRICT;

ALTER TABLE sys_ai_document_node_thread
    ADD CONSTRAINT fk_sys_ai_document_node_thread_task
    FOREIGN KEY (task_id) REFERENCES sys_ai_document_task(id)
    ON DELETE CASCADE
    ON UPDATE RESTRICT;

ALTER TABLE sys_ai_document_node_thread
    ADD CONSTRAINT fk_sys_ai_document_node_thread_user
    FOREIGN KEY (user_id) REFERENCES sys_user(id)
    ON DELETE CASCADE
    ON UPDATE RESTRICT;

ALTER TABLE sys_ai_document_node_message
    ADD CONSTRAINT fk_sys_ai_document_node_message_thread
    FOREIGN KEY (thread_id) REFERENCES sys_ai_document_node_thread(id)
    ON DELETE CASCADE
    ON UPDATE RESTRICT;
...
ALTER TABLE sys_article
    ADD CONSTRAINT fk_sys_article_series
    FOREIGN KEY (series_id) REFERENCES sys_series(id)
    ON DELETE SET NULL
    ON UPDATE RESTRICT;
...
CREATE TRIGGER trg_sys_series_update_time
    BEFORE UPDATE ON sys_series
    FOR EACH ROW
    EXECUTE FUNCTION set_update_time();
...
CREATE TRIGGER trg_sys_ai_document_node_thread_update_time
    BEFORE UPDATE ON sys_ai_document_node_thread
    FOR EACH ROW
    EXECUTE FUNCTION set_update_time();
...
INSERT INTO sys_dict (id, name, type, status, remark, create_time, update_time, sort)
VALUES (38, '下载分类', 'download_category', 1, '下载资源分类', '2025-02-14 08:47:01', '2025-02-14 08:47:01', 0);
...
(141, '68', 'series', '/article/series/index', '系列管理', 3, 'Collection', 'MENU', '2026-08-27 10:00:00', NULL, '', '', 0, '', 0),
(142, '141', '', '', '系列列表', 1, '', 'BUTTON', '2026-08-27 10:00:00', NULL, '', '', 1, 'sys:series:list', 0),
(143, '141', '', '', '新增系列', 2, '', 'BUTTON', '2026-08-27 10:00:00', NULL, '', '', 1, 'sys:series:add', 0),
(144, '141', '', '', '修改系列', 3, '', 'BUTTON', '2026-08-27 10:00:00', NULL, '', '', 1, 'sys:series:update', 0),
(145, '141', '', '', '删除系列', 4, '', 'BUTTON', '2026-08-27 10:00:00', NULL, '', '', 1, 'sys:series:delete', 0);
...
(11, 1, 13),(12, 1, 14),(13, 1, 16),(14, 1, 17),(15, 1, 18),(16, 1, 19),(17, 1, 27),(18, 1, 28),(19, 1, 29),(22, 1, 32),
(23, 1, 33),(24, 1, 39),(25, 1, 40),(26, 1, 41),(27, 1, 43),(28, 1, 44),(29, 1, 48),(30, 1, 49),
...
(117, 1, 141),(118, 1, 142),(119, 1, 143),(120, 1, 144),(121, 1, 145);
